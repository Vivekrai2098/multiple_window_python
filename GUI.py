...
def shyam():
    ...
    # this is another function which submit the value
    def submit():
        if en.get()=="" or en2.get()=="":
            ...
            if pinname=="":
                ...
                ko=messagebox.showinfo("info","congratulation you register")
                if ko==True:
                    print("hello")
                else:
                    top.destroy()
...
te=Frame(h)
te.pack(side=TOP)
Label(te,text="Welcome to the SBI ATM",fg="white",font=("arial",33),bg="blue").pack()
...
# this is the end of entry
bt=Button(text="Register your self",bd=5,command=shyam)
bt.place(x=20,y=110)


# cash deposite fuction
def depo():
    top2 = Toplevel()
    top2.maxsize(width=650, height=650)
    top2.minsize(width=640, height=600)
    head2 = Label(top2, text="Welcome to CASH DEPOSITE", bg="orange", fg="black", font=("", 26)).pack()
    # cash deposite
        ...
        if len(pinname)<1:
            ...
            for i in pinname:
                if i==do:
                    ...
                    print(nameamount)
                    lp=messagebox.showinfo("succefully","money deposite succefully")
                    if lp==True:
                        print("yes")
                    else:
                        top2.destroy()
...
# this is the function of cash withdrwal
def cash():
    top3 = Toplevel()
        ...
        if len(nameamount) < 1:
            ...
            for i in pinname:
                if i == dos:
                    ...
                    textho2.config(text="Welcome back Mr:=" + pinname.get(i) + "\nthanks for using SBI\n widthdrwal amount:-" + ky)
                    for i in nameamount:
                        width=nameamount.get(i)-so
                        nameamount[i]=width
                        print(nameamount)
                    lp = messagebox.showinfo("succefully", "money Widthdrwal succefully")
                    if lp == True:
                        print("yes")
                    else:
                        top3.destroy()
    ...
    pin2 = Label(top3, text="Enter your Withdrawal Amount:-")
...
def checkb():
        ...
        if len(nameamount) < 1:
            ...
            for i in pinname:
                if i == dosh:
                    ...
                    textho3.config(
                        text="Welcome back Mr:=" + pinname.get(i) + "\nthanks for using SBI\nCash Amount:-"+ho+"Ruppes")
    ...
    # pin3 = Label(top4, text="Enter your Withdrawal Amount:-")
    # pin3.place(x=2, y=200)
...
bu=Frame(h)
bu.pack(side=BOTTOM)
Label(bu,text="Made by VIVEK @Copright issues..").pack()
h.mainloop()